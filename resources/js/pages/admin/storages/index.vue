...
const store = async (formData) => {
    try {
        await axios.post("http://localhost:8000/api/admin/storage", formData);
...
const update = async (formData) => {
    try {
        await axios.patch(
            `http://localhost:8000/api/admin/storage/${formData.id}`,
            formData
        );
...
const remove = async (item) => {
    try {
        await axios.delete(
            `http://localhost:8000/api/admin/storage/${item.id}`
        );
        await getStorage();
